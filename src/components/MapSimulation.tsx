...
import { GasMarker } from "./simulation/GasMarker";
...
function MapController({ center, isSidebarOpen }: { center: L.LatLng | null, isSidebarOpen?: boolean }) {
  const map = useMap();
  useEffect(() => {
...
export default function MapSimulation() {
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden">
      {/* SIDEBAR WRAPPER */}
      <div
        className={`relative h-full transition-all duration-300 ease-in-out flex-shrink-0 z-[20] ${
      ...
      >
        ...
        <button
          ...
          className={`absolute top-1/2 -translate-y-1/2 w-6 h-16 bg-white border border-slate-200 rounded-r-md shadow-md flex items-center justify-center text-slate-500 hover:bg-slate-50 hover:text-blue-600 focus:outline-none z-[21] transition-transform duration-300 ease-in-out ${
      ...
      <div className="flex-1 relative bg-white flex flex-col">
        ...
        {currentMap?.status === "ready" ? (
          ...
          >
            ...
            <MapController center={targetRaw} isSidebarOpen={isSidebarOpen} />
            ...
            {/* Click Marker */}
            {clickedRaw && isCalibrated && (
              weatherActive ? (
                <GasMarker center={clickedRaw} angle={weatherData.angle} scaleX={scale.x} />
              ) : (
            ...
            {targetRaw && (
              weatherActive ? (
                <GasMarker center={targetRaw} angle={weatherData.angle} scaleX={scale.x} />
              ) : (
