...
export const CalibrationPanel = ({
  ...
  p1, setP1,
  p2, setP2,
  isSelectingFor, setIsSelectingFor,
  calculateCalibration
}: any) => {
  return (
    <div className={`space-y-4 p-4 rounded-xl border ${isCalibrated ? 'bg-slate-50 border-slate-200 opacity-80' : 'bg-blue-50/50 border-blue-200 shadow-sm'}`}>
      <div className="flex items-center justify-between border-b border-slate-200 pb-2 cursor-pointer" onClick={() => setShowCalibration(!showCalibration)}>
        ...
        <div className="flex items-center gap-2">
          {isCalibrated && <Button variant="outline" onClick={(e: any) => { e.stopPropagation(); setIsCalibrated(false); setShowCalibration(true); }} className="h-7 text-xs px-2">Căn lại</Button>}
          <span className="text-slate-400 text-xs">{showCalibration ? '▼' : '▲'}</span>
        </div>
      </div>
